...
//              System.out.println(currentPosition);
	...
	public SegmentNode popSegmentNode(SegmentParams params, ProfileData profileData) {
		...
		if (params.getIntegrationProfileId() != null) {
			if (profileData != null && profileData.getIntegrationProfile() != null
				...
				if (s != null && params.getLineStr() != null && params.getLineStr().startsWith(s.getName())) {
	...
	public TestStepXMLOutput getXMLs(TestStepXMLParams params, ProfileData profileData) throws Exception {
		if (profileData != null && profileData.getIntegrationProfile() != null) {
			ConformanceProfile cp = profileData.getIntegrationProfile()
					.findConformanceProfileById(params.getConformanceProfileId());
						...
						if (iPathList.length == 1) {
							...
							for (int i = 0; i < iPathList.length; i++) {
								...
								if (i == iPathList.length - 1) {
									Element segmentSTDElm = stdXMLDoc
											.createElement(iPath.substring(0, iPath.lastIndexOf("[")));
									Element segmentNISTElm = nistXMLDoc
											.createElement(iPath.substring(0, iPath.lastIndexOf("[")));
									this.generateSTDSegment(segmentSTDElm, segment, stdXMLDoc,
											profileData.getIntegrationProfile());
									...
									NodeList nodeListSTDgroups = parentSTDElm
											.getElementsByTagName(rootName + "." + groupName);
									NodeList nodeListNISTgroups = parentNISTElm
											.getElementsByTagName(rootName + "." + groupName);
	...
	public TestStepSupplementsOutput getSupplements(TestStepSupplementsParams params, ProfileData profileData)
			throws Exception {
		...
		if (params.getJdXSL() != null && !params.getJdXSL().equals("")) {
			if (params.getJdXSL().equals("IZ52-IZ33NF-IZ33TM-IZ33PD-JurorDocument")) {
				...
				if (qak2.equals("OK")) {
					String jdTemplate = IOUtils
							.toString(classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_OK.txt"));
					...
					jdTemplate = jdTemplate.replace("$PatientName$",
							this.generateName(firstPatientName, middlePatientName, lastPatientName));
					...
					if (gender.equals("M"))
						jdTemplate = jdTemplate.replace("$Gender$", "Male");
					else if (gender.equals("F"))
						jdTemplate = jdTemplate.replace("$Gender$", "Female");
					else
						jdTemplate = jdTemplate.replace("$Gender$", gender);

					String immunizationScheduleUsed = this.findDataByPathAndCondition(nistXMLDom, "OBX", "OBX.3.1",
							"59779-9", "OBX.5.2", 0);
					...
					if (COMPLETE_HISTORYGroups != null && COMPLETE_HISTORYGroups.getLength() > 0) {
						for (int i = 0; i < COMPLETE_HISTORYGroups.getLength(); i++) {
							...
							if (value_RXA_5_1.equals("998")) {
								...
								if (obxSegments != null && obxSegments.getLength() > 0) {
									for (int j = obxSegments.getLength() - 1; j > -1; j--) {
										...
										if (this.findDataByPath(obxSegment, "OBX.3.1").equals("30956-7")) {
											immunizationForecastVaccineGroup = this.findDataByPath(obxSegment,
													"OBX.5.2");
											String trHTML = "<tr>";
											if (immunizationForecastVaccineGroup.equals("NOT_Found")
													|| immunizationForecastVaccineGroup.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ immunizationForecastVaccineGroup + "</td>";
											}
											if (immunizationForecastDueDate.equals("NOT_Found")
													|| immunizationForecastDueDate.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ this.changeDateFormat(immunizationForecastDueDate) + "</td>";
											}
											if (immunizationForecastEarliestDatetoGive.equals("NOT_Found")
													|| immunizationForecastEarliestDatetoGive.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ this.changeDateFormat(immunizationForecastEarliestDatetoGive)
														+ "</td>";
											}
											if (immunizationForecastLatestDatetoGive.equals("NOT_Found")
													|| immunizationForecastLatestDatetoGive.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ this.changeDateFormat(immunizationForecastLatestDatetoGive)
														+ "</td>";
											}
											trHTML = trHTML
											...
											immunizationForecastEarliestDatetoGive = this.findDataByPath(obxSegment,
													"OBX.5.1");
										} else if (this.findDataByPath(obxSegment, "OBX.3.1").equals("59777-3")) {
											immunizationForecastLatestDatetoGive = this.findDataByPath(obxSegment,
													"OBX.5.1");
										}
									}
								}
								...
								String immunizationHistoryVaccineAdministered = this
										.findDataByPath(COMPLETE_HISTORYGroup, "RXA.5.2");
								String immunizationHistoryDateAdministered = this.findDataByPath(COMPLETE_HISTORYGroup,
										"RXA.3");
								String immunizationHistoryValidDose = "";
								String immunizationHistoryValidityReason = "";
								String immunizationHistoryCompletionStatus = this.findDataByPath(COMPLETE_HISTORYGroup,
										"RXA.20");
								...
								if (obxSegments != null && obxSegments.getLength() > 0) {
									for (int j = obxSegments.getLength() - 1; j > -1; j--) {
										Element obxSegment = (Element) obxSegments.item(j);
										if (this.findDataByPath(obxSegment, "OBX.3.1").equals("30956-7")) {
											immunizationHistoryVaccineGroup = this.findDataByPath(obxSegment,
													"OBX.5.2");
											String trHTML = "<tr>";

											if (immunizationHistoryVaccineGroup.equals("NOT_Found")
													|| immunizationHistoryVaccineGroup.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ immunizationHistoryVaccineGroup + "</td>";
											}

											if (immunizationHistoryVaccineAdministered.equals("NOT_Found")
													|| immunizationHistoryVaccineAdministered.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ immunizationHistoryVaccineAdministered + "</td>";
											}

											if (immunizationHistoryDateAdministered.equals("NOT_Found")
													|| immunizationHistoryDateAdministered.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ this.changeDateFormat(immunizationHistoryDateAdministered)
														+ "</td>";
											}

											if (immunizationHistoryValidDose.equals("NOT_Found")
													|| immunizationHistoryValidDose.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												if (immunizationHistoryValidDose.equals("Y")) {
													trHTML = trHTML + "<td style=\"text-align: center;\">" + "YES"
															+ "</td>";
												} else if (immunizationHistoryValidDose.equals("N")) {
													trHTML = trHTML + "<td style=\"text-align: center;\">" + "NO"
															+ "</td>";
												} else {
													trHTML = trHTML
															+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											...
											if (immunizationHistoryValidityReason.equals("NOT_Found")
													|| immunizationHistoryValidityReason.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												trHTML = trHTML + "<td style=\"text-align: center;\">"
														+ immunizationHistoryValidityReason + "</td>";
											}

											if (immunizationHistoryCompletionStatus.equals("NOT_Found")
													|| immunizationHistoryCompletionStatus.equals("")) {
												trHTML = trHTML
														+ "<td style=\"text-align: center;\" bgcolor=\"#D2D2D2\"></td>";
											} else {
												if (immunizationHistoryCompletionStatus.equals("CP")) {
													trHTML = trHTML + "<td style=\"text-align: center;\">" + "Complete"
															+ "</td>";
												} else if (immunizationHistoryCompletionStatus.equals("RE")) {
													trHTML = trHTML + "<td style=\"text-align: center;\">" + "Refused"
															+ "</td>";
											...
											immunizationHistoryValidityReason = this.findDataByPath(obxSegment,
													"OBX.5.1");
										}
									}
								}
					...
					String jdTemplate = IOUtils
							.toString(classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_NF.txt"));
					jdTemplate = jdTemplate.replace("$testcasename$", testCaseName);
					...
					String jdTemplate = IOUtils
							.toString(classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_PD.txt"));
					jdTemplate = jdTemplate.replace("$testcasename$", testCaseName);
					String patientIdentifier = this.findDataByPath(nistXMLDom, "QPD.3.1");
					jdTemplate = jdTemplate.replace("$patientIdentifier$", patientIdentifier);
					...
					String jdTemplate = IOUtils
							.toString(classLoader.getResourceAsStream("jdTemplates" + File.separator + "JD_TM.txt"));
					jdTemplate = jdTemplate.replace("$testcasename$", testCaseName);
					String patientIdentifier = this.findDataByPath(nistXMLDom, "QPD.3.1");
					jdTemplate = jdTemplate.replace("$patientIdentifier$", patientIdentifier);
	 ...
	 * @return
	 */
	private String findDataByPathAndCondition(Document nistXMLDom, String path, String path2, String value,
							...
							if (secondfound.getChildNodes() != null && secondfound.getChildNodes().item(0) != null) {
								if (secondfound.getChildNodes().item(0).getNodeValue().equals(value)) {
									...
									if (targets != null && targets.getLength() > index) {
										if (targets.item(index).getChildNodes() != null
												&& targets.item(index).getChildNodes().item(0) != null) {
	...
	public TestStepSupplementXMLsOutput getSupplementXMLs(@RequestBody TestStepSupplementsParams params,
		...
		if (segmentList != null) {
			for (SegmentInstanceData instanceSegment : segmentList) {
				...
				Element oiTargetElm = judgeOrderSpecificOrIndifferent(params.getOrderIndifferentInfoMap(),
						...
						params.getTestDataCategorizationMap(), segmentiPath);
				if (oiTargetElm == null)
					this.createOrderSpecificConstraints(byIDElement, segment, instanceSegment, xmlDoc,
							constraintXMLOutPut, segmentiPositionPath, segmentiPath, segName, profileData, segUsagePath,
							params);
	...
	private void normalizeXML(Element rootElement) {
		Element orderIndifferentElm = (Element) rootElement.getElementsByTagName("OrderIndifferent").item(0);
		...
		if (allConstraints != null) {
			for (int i = 0; i < allConstraints.getLength(); i++) {
				...
				if (constraintElm.getChildNodes() == null || constraintElm.getChildNodes().getLength() == 0) {
		...
		if (allContext != null) {
			for (int i = 0; i < allContext.getLength(); i++) {
				...
				if (contextElm.getChildNodes() == null || contextElm.getChildNodes().getLength() == 0) {
					...
					Set<Element> toBeDel = new HashSet<Element>();
					for (int j = 0; j < contextElm.getChildNodes().getLength(); j++) {
						Element child = (Element) contextElm.getChildNodes().item(j);
						String ref = child.getAttribute("Ref");
						if (ref != null && ref.split("%").length > 1) {
							String key = child.getAttribute("Ref").split("%")[2];
							if (map.containsKey(key)) {
								Element e = map.get(key);
								String c = e.getAttribute("Cardinality");
								if (c == null || c.isEmpty()) {
									e.setAttribute("Cardinality", "2");
								} else {
									if (this.tryParseInt(c)) {
										int num = Integer.parseInt(c) + 1;
										e.setAttribute("Cardinality", "" + num);
									}
								}
								toBeDel.add(child);
							} else {
		...
		if (orderIndifferentElm.getChildNodes() == null || orderIndifferentElm.getChildNodes().getLength() == 0) {
	 ...
	 * @param xmlDoc
	 */
	private Element judgeOrderSpecificOrIndifferent(HashMap<String, OrderIndifferentInfo> orderIndifferentInfoMap,
			String segmentiPositionPath, Element orderIndifferentElement, Document xmlDoc, String segmentName,
			SegmentInstanceData segmentInstanceData, List<SegmentInstanceData> segmentList,
			HashMap<String, Categorization> testDataCategorizationMap, String segmentiPath) {
			...
			for (String positionPath : positionPaths) {
				...
				if (orderIndifferentInfo != null) {
					if (orderIndifferentInfo.isOrderSpecific()) {
						...
						currentElm = this.findContextPattern(currentElm, contextPath.substring(1),
								instancePath.substring(1), orderIndifferentInfo, segmentList,
								testDataCategorizationMap);
					} else {
	...
	private Element findContextPattern(Element currentElm, String contextPath, String instancePath,
		...
		if (currentElm.getNodeName().equals("Contexts")) {
			startNum = this.retrieveRef(((Element) currentElm.getParentNode()).getAttribute("Ref"), 1)
					.split("\\.").length;
		}
		...
		for (int i = 0; i < currentElm.getChildNodes().getLength(); i++) {
			...
			if (listAttrValue.equals(this.subPath(contextPath, startNum))) {
				for (int j = 0; j < contextElm.getChildNodes().getLength(); j++) {
					...
					String refAttrValue = this.retrieveRef(patternElm.getAttribute("Ref"), 1);
				...
				String ref = this.popTriggerAssertion(errorMessageElm, assertionElm, instancePath,
						orderIndifferentInfo.getTriggerInfo(), segmentList, testDataCategorizationMap);
				triggerElm.appendChild(errorMessageElm);
				triggerElm.appendChild(assertionElm);
				patternElm.setAttribute("Ref", ref);
	 ...
	 * @param triggerInfo
	 */
	private String popTriggerAssertion(Element errorMessageElm, Element assertionElm, String instancePath,
			TriggerInfo triggerInfo, List<SegmentInstanceData> segmentList,
			HashMap<String, Categorization> testDataCategorizationMap) {
		String ref = "%" + instancePath + "%";
		if (triggerInfo.getList() != null && triggerInfo.getList().size() == 1) {
			TriggerPath triggerPath = triggerInfo.getList().get(0);
			ref = ref + "[" + this.popOneTriggerAssertion(null, errorMessageElm, assertionElm, triggerPath,
					instancePath, segmentList, testDataCategorizationMap) + "]";
		}
		if (triggerInfo.getList() != null && triggerInfo.getList().size() > 1) {
			if (triggerInfo.getOperation().equals(Operation.AND)) {
				...
				for (TriggerPath triggerPath : triggerInfo.getList()) {
					ref = ref + "[" + this.popOneTriggerAssertion(triggerInfo.getOperation(), errorMessageElm,
							forAllElm, triggerPath, instancePath, segmentList, testDataCategorizationMap) + "]";
				}
				...
				for (TriggerPath triggerPath : triggerInfo.getList()) {
					ref = ref + "[" + this.popOneTriggerAssertion(triggerInfo.getOperation(), errorMessageElm, existElm,
							triggerPath, instancePath, segmentList, testDataCategorizationMap) + "]";
				}
	...
	private String popOneTriggerAssertion(Operation o, Element errorMessageElm, Element parent, TriggerPath triggerPath,
			String instancePath, List<SegmentInstanceData> segmentList,
			HashMap<String, Categorization> testDataCategorizationMap) {
		String result = "";
		SegmentInstanceData segment = this.findTriggerSegmentString(instancePath, triggerPath.getNamePath(),
				this.getPositionIPath(triggerPath.getPositionPath()), segmentList);
		String segmentStr = segment.getLineStr();
		String segmentIPath = segment.getiPath();
		String triggerIPathFull = segmentIPath + "."
				+ this.getPositionIPath(this.getIPathInSegment(triggerPath.getNamePath()));
		Categorization categorization = testDataCategorizationMap.get(this.replaceDot2Dash(triggerIPathFull));

		if (categorization != null && categorization.getTestDataCategorization() != null
				&& categorization.getTestDataCategorization().equals("Value-Test Case Fixed List")) {
			java.util.Collections.sort(categorization.getListData());
			String values = String.join(",", categorization.getListData());
			...
			Element stringListElm = parent.getOwnerDocument().createElement("StringList");
			stringListElm.setAttribute("IgnoreCase", "true");
			stringListElm.setAttribute("Path", this.getPositionIPath(triggerPath.getPositionPath()));
			stringListElm.setAttribute("CSV", values);
			andElm.appendChild(presenceElm);
			andElm.appendChild(stringListElm);
			parent.appendChild(andElm);
			if (o == null)
				errorMessageElm.setTextContent("OrderIndifferent Trigger ERROR: " + triggerPath.getNamePath()
						+ " should be one of '" + values + "' is expected");
			else {
				String existingErrorMessage = errorMessageElm.getTextContent();
				if (existingErrorMessage == null || existingErrorMessage.equals(""))
					errorMessageElm.setTextContent("OrderIndifferent Trigger ERROR: " + "[" + triggerPath.getNamePath()
							+ " should be one of '" + values + "' is expected]");
				else
					errorMessageElm.setTextContent(existingErrorMessage + " " + o.toString() + " " + "[ERROR "
							+ triggerPath.getNamePath() + " should be one of '" + values + "' is expected]");
			}
			result = "#Value-Test Case Fixed List";
			result = result + "#" + this.getPositionIPath(triggerPath.getPositionPath());
			...
			if (o == null)
				...
				if (existingErrorMessage == null || existingErrorMessage.equals(""))
					...
					errorMessageElm.setTextContent(existingErrorMessage + " " + o.toString() + " " + "[ERROR "
							+ triggerPath.getNamePath() + " = '" + value + "' is expected]");
			}
			result = "#Value-Test Case Fixed";
			result = result + "#" + this.getPositionIPath(triggerPath.getPositionPath());
			result = result + "#" + value;
		}
	...
	private String getIPathInSegment(String positionIPath) {
		String result = "";
		String[] split = positionIPath.split("\\.");
	 ...
	 * @return
	 */
	private SegmentInstanceData findTriggerSegmentString(String instancePath, String namePath, String positionIPath,
			List<SegmentInstanceData> segmentList) {
		...
		for (SegmentInstanceData instanceSegment : segmentList) {
			if (instanceSegment.getPositionIPath().equals(result))
	...
	private String findTriggerValue(String segmentStr, String namePath) {
		if (segmentStr != null) {
			...
			for (int i = 0; i < split.length; i++) {
				if (tryParseInt(split[i])) {
					if (fieldPosition == null)
						fieldPosition = Integer.parseInt(split[i]);
					else if (componentPosition == null)
						componentPosition = Integer.parseInt(split[i]);
					else if (subComponentPosition == null)
						subComponentPosition = Integer.parseInt(split[i]);
				}
			}
			...
			if (fieldPosition != null && componentPosition != null && subComponentPosition == null) {
				return this.getComponentStrFromField(this.getFieldStrFromSegment(segmentStr, fieldPosition),
						componentPosition);
			}
			if (fieldPosition != null && componentPosition != null && subComponentPosition != null)
				return this.getSubComponentStrFromField(
						this.getComponentStrFromField(this.getFieldStrFromSegment(segmentStr, fieldPosition),
								componentPosition),
	...
	private void createOrderIndifferntConstraints(Element holderElm, Segment segment,
		...
		int startNum = this.retrieveRef(((Element) holderElm.getParentNode()).getAttribute("Ref"), 1)
				.split("\\.").length;
		...
		for (int i = 0; i < segment.getChildren().size(); i++) {
			...
			for (int j = 0; j < wholeFieldStr.split("\\~").length; j++) {
				...
				if (fieldDT == null || fieldDT.getChildren() == null || fieldDT.getChildren().size() == 0) {
					...
					Categorization cateOfField = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath));
					...
					if (foiInfo != null && !foiInfo.isOrderSpecific() && field.getMax() != null && !field.getMax().equals("0") && !field.getMax().equals("1")) {
						if (cateOfField != null) {
							if (cateOfField.getTestDataCategorization().equals("Indifferent")) {
								...
								this.createNonPresenceCheck(subPath(segmentiPositionPath + fieldiPath, startNum), cateOfField,fieldUsagePath, xmlDoc, field.getName(), holderElm);
							} else if (cateOfField.getTestDataCategorization().equals("Presence-Content Indifferent")
									|| cateOfField.getTestDataCategorization().equals("Presence-Configuration")
									|| cateOfField.getTestDataCategorization().equals("Presence-System Generated")
									|| cateOfField.getTestDataCategorization().equals("Presence-Test Case Proper")) {
							...
							} else if (cateOfField.getTestDataCategorization().equals("Presence Length-Content Indifferent")
									|| cateOfField.getTestDataCategorization().equals("Presence Length-Configuration")
									|| cateOfField.getTestDataCategorization().equals("Presence Length-System Generated")
									|| cateOfField.getTestDataCategorization().equals("Presence Length-Test Case Proper")) {
								this.createPresenceCheck(subPath(segmentiPositionPath + fieldiPath, startNum), cateOfField, fieldUsagePath, xmlDoc, field.getName(), holderElm);
						...
						this.createConstraint(subPath(segmentiPositionPath + fieldiPath, startNum), cateOfField, fieldUsagePath, xmlDoc, field.getName(), fieldStr, holderElm, false);
					}
				} else {
					FieldOrderIndifferentInfo foiInfo = params.getFieldOrderIndifferentInfoMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath));
					if (foiInfo != null && !foiInfo.isOrderSpecific() && field.getMax() != null && !field.getMax().equals("0") && !field.getMax().equals("1")) {
		        		...
		        		if(contextsNL == null || contextsNL.getLength() == 0) {
		        			contextsElm = xmlDoc.createElement("OrderIndifferent");
		        			holderElm.getParentNode().appendChild(contextsElm);
		        		} else {
		        			contextsElm = (Element)contextsNL.item(0);
		        		}
	        			Element contextElm = contextsElm.getOwnerDocument().createElement("Context");
	        		    contextElm.setAttribute("List", subPath(segmentiPositionPath + "." + (i + 1) + "[*]", startNum));
	        		    Element patternElm = contextsElm.getOwnerDocument().createElement("Pattern");
	        	        Element triggerElm = contextsElm.getOwnerDocument().createElement("Trigger");
	        	        Element constraintsElm = contextsElm.getOwnerDocument().createElement("Constraints");
	        	        patternElm.appendChild(triggerElm);
	        	        patternElm.appendChild(constraintsElm);
	        	        contextElm.appendChild(patternElm);
	        		    
	        		    Element errorMessageElm = triggerElm.getOwnerDocument().createElement("ErrorMessage");
	        	        Element assertionElm = triggerElm.getOwnerDocument().createElement("Assertion");
						Element forAllElm = assertionElm.getOwnerDocument().createElement("FORALL");
	        	        
	        	        triggerElm.appendChild(errorMessageElm);
	        	        triggerElm.appendChild(assertionElm);
	        	        
	        	        int startNum2 = startNum + contextElm.getAttribute("List").split("\\.").length;
	        	        for (int k = 0; k < fieldDT.getChildren().size(); k++) {
							...
							Datatype componentDT = profileData.getIntegrationProfile().findDatatypeById(c.getDatatypeId());
							if (componentDT == null || componentDT.getChildren() == null || componentDT.getChildren().size() == 0) {
								Categorization cateOfComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath));
								this.createConstraint(subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum2), cateOfComponent, componentUsagePath, xmlDoc, c.getName(), componentStr, constraintsElm, false);
								constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentStr);
								constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentUsagePath);
								
								if (cateOfComponent != null && cateOfComponent.getTestDataCategorization() != null && cateOfComponent.getTestDataCategorization().equals("Value-Test Case Fixed")) {
									Element presenceElm = assertionElm.getOwnerDocument().createElement("Presence");
									presenceElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum2));
									Element plainTextElm = assertionElm.getOwnerDocument().createElement("PlainText");
									plainTextElm.setAttribute("IgnoreCase", "true");
									plainTextElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum2));
									plainTextElm.setAttribute("Text", componentStr);
									forAllElm.appendChild(presenceElm);
									forAllElm.appendChild(plainTextElm);
									String existingErrorMessage = errorMessageElm.getTextContent();
									if (existingErrorMessage == null || existingErrorMessage.equals(""))
										errorMessageElm.setTextContent("OrderIndifferent Trigger ERROR: " + "[" + this.modifyFormIPath(cateOfComponent.getiPath()) + " (" + c.getName() + ")" + " = '" + componentStr + "' is expected]");
									else
										errorMessageElm.setTextContent(existingErrorMessage + " AND " + "[" + this.modifyFormIPath(cateOfComponent.getiPath()) + " (" + c.getName() + ")" + " = '" + componentStr + "' is expected]");
									
								} else if (cateOfComponent != null && cateOfComponent.getTestDataCategorization() != null && cateOfComponent.getTestDataCategorization().equals("Value-Test Case Fixed List")) {
									java.util.Collections.sort(cateOfComponent.getListData());
									String values = String.join(",", cateOfComponent.getListData());
									Element presenceElm = assertionElm.getOwnerDocument().createElement("Presence");
									presenceElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum2));
									Element stringListElm = assertionElm.getOwnerDocument().createElement("StringList");
									stringListElm.setAttribute("IgnoreCase", "true");
									stringListElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum2));
									stringListElm.setAttribute("CSV", values);
									forAllElm.appendChild(presenceElm);
									forAllElm.appendChild(stringListElm);
					
									String existingErrorMessage = errorMessageElm.getTextContent();
									if (existingErrorMessage == null || existingErrorMessage.equals(""))
										errorMessageElm.setTextContent("OrderIndifferent Trigger ERROR: " + "[" + this.modifyFormIPath(cateOfComponent.getiPath()) + " (" + c.getName() + ")" + " should be one of '" + values + "' is expected]");
									else
										errorMessageElm.setTextContent(existingErrorMessage + " AND " + "["+ this.modifyFormIPath(cateOfComponent.getiPath()) + " (" + c.getName() + ")" + " should be one of '" + values + "' is expected]");
								}
								...
								for (int l = 0; l < componentDT.getChildren().size(); l++) {
									...
									Categorization cateOfSubComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath));
									this.createConstraint(subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum2), cateOfSubComponent, subComponentUsagePath, xmlDoc, sc.getName(), subcomponentStr, constraintsElm, false);
									constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath), subcomponentStr);
									constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath),subComponentUsagePath);
									
									if (cateOfSubComponent != null && cateOfSubComponent.getTestDataCategorization() != null && cateOfSubComponent.getTestDataCategorization().equals("Value-Test Case Fixed")) {
										Element presenceElm = assertionElm.getOwnerDocument().createElement("Presence");
										presenceElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum2));
										Element plainTextElm = assertionElm.getOwnerDocument().createElement("PlainText");
										plainTextElm.setAttribute("IgnoreCase", "true");
										plainTextElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum2));
										plainTextElm.setAttribute("Text", subcomponentStr);
										forAllElm.appendChild(presenceElm);
										forAllElm.appendChild(plainTextElm);
										String existingErrorMessage = errorMessageElm.getTextContent();
										if (existingErrorMessage == null || existingErrorMessage.equals(""))
											errorMessageElm.setTextContent("OrderIndifferent Trigger ERROR: " + "[" + this.modifyFormIPath(cateOfSubComponent.getiPath()) + " (" + sc.getName() + ")" + " = '" + subcomponentStr + "' is expected]");
										else
											errorMessageElm.setTextContent(existingErrorMessage + " AND " + "[" + this.modifyFormIPath(cateOfSubComponent.getiPath()) + " (" + sc.getName() + ")" + " = '" + subcomponentStr + "' is expected]");
										
									} else if (cateOfSubComponent != null && cateOfSubComponent.getTestDataCategorization() != null && cateOfSubComponent.getTestDataCategorization().equals("Value-Test Case Fixed List")) {
										java.util.Collections.sort(cateOfSubComponent.getListData());
										String values = String.join(",", cateOfSubComponent.getListData());
										Element presenceElm = assertionElm.getOwnerDocument().createElement("Presence");
										presenceElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum2));
										Element stringListElm = assertionElm.getOwnerDocument().createElement("StringList");
										stringListElm.setAttribute("IgnoreCase", "true");
										stringListElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum2));
										stringListElm.setAttribute("CSV", values);
										forAllElm.appendChild(presenceElm);
										forAllElm.appendChild(stringListElm);
										...
										String existingErrorMessage = errorMessageElm.getTextContent();
										if (existingErrorMessage == null || existingErrorMessage.equals(""))
											errorMessageElm.setTextContent("OrderIndifferent Trigger ERROR: " + "[" + this.modifyFormIPath(cateOfSubComponent.getiPath()) + " (" + sc.getName() + ")" + " should be one of '" + values + "' is expected]");
										else
											errorMessageElm.setTextContent(existingErrorMessage + " AND " + "["+ this.modifyFormIPath(cateOfSubComponent.getiPath()) + " (" + sc.getName() + ")" + " should be one of '" + values + "' is expected]");
									}
								}
							}
						}
	        	        if(forAllElm.getChildNodes() != null && forAllElm.getChildNodes().getLength() > 0) {
	        	        	assertionElm.appendChild(forAllElm);	
	        	        	contextsElm.appendChild(contextElm);
	        	        }
					} else {
						for (int k = 0; k < fieldDT.getChildren().size(); k++) {
							Component c = fieldDT.getChildren().get(k);
							...
							if (componentDT == null || componentDT.getChildren() == null || componentDT.getChildren().size() == 0) {
								Categorization cateOfComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath));
								this.createConstraint(subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum), cateOfComponent, componentUsagePath, xmlDoc, c.getName(), componentStr, holderElm, false);
								constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentStr);
								constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentUsagePath);
							} else {
								for (int l = 0; l < componentDT.getChildren().size(); l++) {
									...
									String subcomponentStr = this.getSubComponentStrFromField(componentStr, (l + 1));
									Categorization cateOfSubComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath));
									this.createConstraint(subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum), cateOfSubComponent, subComponentUsagePath, xmlDoc, sc.getName(), subcomponentStr, holderElm, false);
									constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath), subcomponentStr);
									constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath), subComponentUsagePath);
								}
							}
	...
	private String retrieveRef(String attribute, int key) {
		return attribute.split("%")[key];
	}

	private void createOrderSpecificConstraints(Element holderElm, Segment segment, SegmentInstanceData instanceSegment,
			Document xmlDoc, ConstraintXMLOutPut constraintXMLOutPut, String segmentiPositionPath, String segmentiPath,
			String segName, ProfileData profileData, String segUsagePath, TestStepSupplementsParams params) {
		DynamicInfo dynamicInfo = this.findDynamicInfo(segment, instanceSegment.getLineStr());
		for (int i = 0; i < segment.getChildren().size(); i++) {
			Field field = segment.getChildren().get(i);
			String wholeFieldStr = this.getFieldStrFromSegment(segName, instanceSegment, (i + 1));
					...
					if (foiInfo != null && !foiInfo.isOrderSpecific() && field.getMax() != null && !field.getMax().equals("0") && !field.getMax().equals("1")) {
						if (cateOfField != null) {
							if (cateOfField.getTestDataCategorization().equals("Indifferent")) {
								...
								this.createNonPresenceCheck(segmentiPositionPath + fieldiPath, cateOfField,fieldUsagePath, xmlDoc, field.getName(), holderElm);
							} else if (cateOfField.getTestDataCategorization().equals("Presence-Content Indifferent")
									|| cateOfField.getTestDataCategorization().equals("Presence-Configuration")
									|| cateOfField.getTestDataCategorization().equals("Presence-System Generated")
									|| cateOfField.getTestDataCategorization().equals("Presence-Test Case Proper")) {
								this.createPresenceCheck(segmentiPositionPath + fieldiPath, cateOfField, fieldUsagePath, xmlDoc, field.getName(), holderElm);
							} else if (cateOfField.getTestDataCategorization().equals("Presence Length-Content Indifferent")
									|| cateOfField.getTestDataCategorization().equals("Presence Length-Configuration")
									|| cateOfField.getTestDataCategorization().equals("Presence Length-System Generated")
									|| cateOfField.getTestDataCategorization().equals("Presence Length-Test Case Proper")) {
								this.createPresenceCheck(segmentiPositionPath + fieldiPath, cateOfField, fieldUsagePath, xmlDoc, field.getName(), holderElm);
								this.createLengthCheck(segmentiPositionPath + fieldiPath, cateOfField, fieldUsagePath, xmlDoc, field.getName(), fieldStr, holderElm);
							} else if (cateOfField.getTestDataCategorization().equals("Value-Test Case Fixed")) {
								this.createPresenceCheck(segmentiPositionPath + fieldiPath, cateOfField, fieldUsagePath, xmlDoc, field.getName(), holderElm);
								this.createPlainTextCheck(segmentiPositionPath + "." + (i + 1) + "[*]", cateOfField, fieldUsagePath, xmlDoc, field.getName(), fieldStr, holderElm, true);
							} else if (cateOfField.getTestDataCategorization().equals("Value-Test Case Fixed List")) {
								this.createPresenceCheck(segmentiPositionPath + fieldiPath, cateOfField, fieldUsagePath, xmlDoc, field.getName(), holderElm);
								this.createStringListCheck(segmentiPositionPath + "." + (i + 1) + "[*]", cateOfField, fieldUsagePath, xmlDoc, field.getName(), fieldStr, holderElm, true);
							}
						}
					} else {
						this.createConstraint(segmentiPositionPath + fieldiPath, cateOfField, fieldUsagePath, xmlDoc, field.getName(), fieldStr, holderElm, false);
					}
				} else {
					FieldOrderIndifferentInfo foiInfo = params.getFieldOrderIndifferentInfoMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath));
					if (foiInfo != null && !foiInfo.isOrderSpecific() && field.getMax() != null && !field.getMax().equals("0") && !field.getMax().equals("1")) {
		        		...
		        		if(orderIndifferents == null || orderIndifferents.getLength() == 0) {
		        			orderIndifferentElm = xmlDoc.createElement("OrderIndifferent");
		        			xmlDoc.appendChild(orderIndifferentElm);
		        		} else {
		        			orderIndifferentElm = (Element)orderIndifferents.item(0);
		        		}
	        			Element contextElm = orderIndifferentElm.getOwnerDocument().createElement("Context");
	        		    contextElm.setAttribute("List", segmentiPositionPath + "." + (i + 1) + "[*]");
	        		    Element patternElm = orderIndifferentElm.getOwnerDocument().createElement("Pattern");
	        	        Element triggerElm = orderIndifferentElm.getOwnerDocument().createElement("Trigger");
	        	        Element constraintsElm = orderIndifferentElm.getOwnerDocument().createElement("Constraints");
	        	        patternElm.appendChild(triggerElm);
	        	        patternElm.appendChild(constraintsElm);
	        	        contextElm.appendChild(patternElm);
	        	        ...
	        	        int startNum = contextElm.getAttribute("List").split("\\.").length;
	        	        for (int k = 0; k < fieldDT.getChildren().size(); k++) {
							Component c = fieldDT.getChildren().get(k);
							String componentUsagePath = fieldUsagePath + '-' + c.getUsage();
							String componentiPath = "." + (k + 1) + "[1]";
							...
							if (componentDT == null || componentDT.getChildren() == null || componentDT.getChildren().size() == 0) {
								Categorization cateOfComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath));
								this.createConstraint(subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum), cateOfComponent, componentUsagePath, xmlDoc, c.getName(), componentStr, constraintsElm, false);
								constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentStr);
								constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentUsagePath);
								
								if (cateOfComponent != null && cateOfComponent.getTestDataCategorization() != null && cateOfComponent.getTestDataCategorization().equals("Value-Test Case Fixed")) {
									Element presenceElm = assertionElm.getOwnerDocument().createElement("Presence");
									presenceElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum));
									Element plainTextElm = assertionElm.getOwnerDocument().createElement("PlainText");
									plainTextElm.setAttribute("IgnoreCase", "true");
									plainTextElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum));
									plainTextElm.setAttribute("Text", componentStr);
									forAllElm.appendChild(presenceElm);
									forAllElm.appendChild(plainTextElm);
									String existingErrorMessage = errorMessageElm.getTextContent();
									...
									Element stringListElm = assertionElm.getOwnerDocument().createElement("StringList");
									stringListElm.setAttribute("IgnoreCase", "true");
									stringListElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath, startNum));
									stringListElm.setAttribute("CSV", values);
									forAllElm.appendChild(presenceElm);
									forAllElm.appendChild(stringListElm);
								...
								for (int l = 0; l < componentDT.getChildren().size(); l++) {
									...
									String subcomponentStr = this.getSubComponentStrFromField(componentStr, (l + 1));
									Categorization cateOfSubComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath));
									this.createConstraint(subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum), cateOfSubComponent, subComponentUsagePath, xmlDoc, sc.getName(), subcomponentStr, constraintsElm, false);
									constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath), subcomponentStr);
									constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath + subcomponentiPath),subComponentUsagePath);
									
									if (cateOfSubComponent != null && cateOfSubComponent.getTestDataCategorization() != null && cateOfSubComponent.getTestDataCategorization().equals("Value-Test Case Fixed")) {
										Element presenceElm = assertionElm.getOwnerDocument().createElement("Presence");
										presenceElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum));
										Element plainTextElm = assertionElm.getOwnerDocument().createElement("PlainText");
										plainTextElm.setAttribute("IgnoreCase", "true");
										plainTextElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum));
										plainTextElm.setAttribute("Text", subcomponentStr);
										forAllElm.appendChild(presenceElm);
										forAllElm.appendChild(plainTextElm);
										String existingErrorMessage = errorMessageElm.getTextContent();
										...
										Element stringListElm = assertionElm.getOwnerDocument().createElement("StringList");
										stringListElm.setAttribute("IgnoreCase", "true");
										stringListElm.setAttribute("Path", subPath(segmentiPositionPath + fieldiPath + componentiPath + subcomponentiPath, startNum));
										stringListElm.setAttribute("CSV", values);
										forAllElm.appendChild(presenceElm);
										forAllElm.appendChild(stringListElm);
	        	        ...
	        	        if(forAllElm.getChildNodes() != null && forAllElm.getChildNodes().getLength() > 0) {
	        	        	assertionElm.appendChild(forAllElm);	
	        	        	orderIndifferentElm.appendChild(contextElm);
	        	        }
					} else {
						for (int k = 0; k < fieldDT.getChildren().size(); k++) {
							Component c = fieldDT.getChildren().get(k);
							...
							if (componentDT == null || componentDT.getChildren() == null || componentDT.getChildren().size() == 0) {
								Categorization cateOfComponent = params.getTestDataCategorizationMap().get(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath));
								this.createConstraint(segmentiPositionPath + fieldiPath + componentiPath, cateOfComponent, componentUsagePath, xmlDoc, c.getName(), componentStr, holderElm, false);
								constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentStr);
								constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(segmentiPath + fieldiPath + componentiPath), componentUsagePath);
							} else {
								for (int l = 0; l < componentDT.getChildren().size(); l++) {
									...
									this.createConstraint(
											...
											subcomponentStr, holderElm, false);
									constraintXMLOutPut.getCategorizationsDataMap().put(this.replaceDot2Dash(
													segmentiPath + fieldiPath + componentiPath + subcomponentiPath),
													subcomponentStr);
									constraintXMLOutPut.getCategorizationsUsageMap().put(this.replaceDot2Dash(
													segmentiPath + fieldiPath + componentiPath + subcomponentiPath),
	...
	private void createConstraint(String iPositionPath, Categorization cate, String usagePath, Document xmlDoc,
		...
		if (cate != null) {
			if (cate.getTestDataCategorization().equals("Indifferent")) {
				...
				this.createPresenceCheck(iPositionPath, cate, usagePath, xmlDoc, nodeName, holderElm);
				this.createStringListCheck(iPositionPath, cate, usagePath, xmlDoc, nodeName, value, holderElm,
						isAtLeastOnce);
			}
		}
	}

	private void createStringListCheck(String iPositionPath, Categorization cate, String usagePath, Document xmlDoc,
			String nodeName, String value, Element byIDElm, boolean isAtLeastOnce) {
		if (cate != null && cate.getListData() != null && cate.getListData().size() > 0) {
			...
			elmDescription
					.appendChild(xmlDoc.createTextNode("Invalid content (based on test case fixed data). The value at "
			...
			if (isAtLeastOnce)
				elmStringList.setAttribute("AtLeastOnce", "true");
	...
	private void createPlainTextCheck(String iPositionPath, Categorization cate, String usagePath, Document xmlDoc,
			String nodeName, String value, Element byIDElm, boolean isAtLeastOnce) {
		if (cate != null && value != null && !value.equals("")) {
			...
			if (isAtLeastOnce)
				elmPlainText.setAttribute("AtLeastOnce", "true");
	...
	private void createLengthCheck(String iPositionPath, Categorization cate, String usagePath, Document xmlDoc,
			String nodeName, String value, Element byIDElm) {
		Element elmConstraint = xmlDoc.createElement("Constraint");
		Element elmReference = xmlDoc.createElement("Reference");
		elmReference.setAttribute("Source", "testcase");
		elmReference.setAttribute("GeneratedBy", "Test Case Authoring & Management Tool(TCAMT)");
		...
		Element elmDescription = xmlDoc.createElement("Description");
		elmDescription
				.appendChild(xmlDoc.createTextNode("Content does not meet the minimum length requirement. The value at "
						+ this.modifyFormIPath(cate.getiPath()) + " (" + nodeName + ") is expected to be at minimum '"
	...
	private void createPresenceCheck(String iPositionPath, Categorization cate, String usagePath, Document xmlDoc,
			String nodeName, Element byIDElm) {
		...
		if (!usageCheck) {
			...
			elmConstraint.setAttribute("Target", iPositionPath);
			Element elmDescription = xmlDoc.createElement("Description");
			elmDescription.appendChild(xmlDoc.createTextNode("Expected content is missing. The empty value at "
					+ this.modifyFormIPath(cate.getiPath()) + " (" + nodeName + ") is expected to be present."));
			Element elmAssertion = xmlDoc.createElement("Assertion");
	...
	private void createNonPresenceCheck(String iPositionPath, Categorization cate, String usagePath, Document xmlDoc,
			String nodeName, Element byIDElm) {
		Element elmConstraint = xmlDoc.createElement("Constraint");
		Element elmReference = xmlDoc.createElement("Reference");
		elmReference.setAttribute("Source", "testcase");
		...
		elmDescription.appendChild(
				xmlDoc.createTextNode("Unexpected content found. The value at " + this.modifyFormIPath(cate.getiPath())
	...
	private String generateMessageContentXML(TestStepSupplementsParams params, ProfileData profileData)
			throws Exception {
			...
			if (cp != null) {
				...
				if (segmentList != null) {
					...
					for (SegmentInstanceData segmentInstanceData : segmentList) {
						...
						DynamicInfo dynamicInfo = this.findDynamicInfo(segment, segmentInstanceData.getLineStr());
...
//									}
									...
									if (fieldDT == null || fieldDT.getChildren() == null
										...
										if (fieldDT != null && fieldDT.getChildren() != null) {
											for (int k = 0; k < fieldDT.getChildren().size(); k++) {
												Component c = fieldDT.getChildren().get(k);
												String componentiPath = "." + (k + 1) + "[1]";
												if (!this.isHideForMessageContentByUsage(segment, c, fieldDT,
														"component", segmentInstanceData.getPositionPath() + "."
																+ (i + 1) + "." + (k + 1),
														cp, profileData)) {
													String componentStr = this.getComponentStrFromField(fieldStr,
															k + 1);
													...
													if (componentDT != null) {
														if (componentDT.getChildren() == null
															...
															Categorization cateOfComponent = params
																	.getTestDataCategorizationMap()
																	.get(this.replaceDot2Dash(segmentiPath + fieldiPath
																			+ componentiPath));
															if (cateOfComponent != null)
																tdcstrOfComponent = cateOfComponent
																		.getTestDataCategorization();

															Element componentElement = xmlDoc
															...
															componentElement.setAttribute("Categrization",
																	tdcstrOfComponent);
															...
															for (int l = 0; l < componentDT.getChildren().size(); l++) {
																...
																if (!this.isHideForMessageContentByUsage2(segment, sc,
																	...
																	String subcomponentStr = this
																			.getSubComponentStrFromField(componentStr,
																					(l + 1));
																	String tdcstrOfSubComponent = "";
																	Categorization cateOfSubComponent = params
																			.getTestDataCategorizationMap()
																			.get(this.replaceDot2Dash(segmentiPath
																					+ fieldiPath + componentiPath
																					+ subcomponentiPath));
																	if (cateOfSubComponent != null)
																		tdcstrOfSubComponent = cateOfSubComponent
																				.getTestDataCategorization();
																	Element subComponentElement = xmlDoc
																			.createElement("SubComponent");
																	subComponentElement.setAttribute("Location",
																			segName + "." + (i + 1) + "." + (k + 1)
																					+ "." + (l + 1));
																	subComponentElement.setAttribute("DataElement",
																			sc.getName());
																	subComponentElement.setAttribute("Data",
																			subcomponentStr);
	...
	private boolean isHideForMessageContentByUsage(Segment segment, Component component, Datatype dt, String type,
	...
	private void generateNISTSegment(Element segmentElm, SegmentInstanceData instanceSegment, Document xmlDoc,
...
//				} 
								...
								if (lineStr.startsWith("OBX")) {
									if ((i + 1) == 2) {
										...
										for (int index = 0; index < componentStrs.length; index++) {
											...
											Element componentElm = xmlDoc
													.createElement(segmentName + "." + (i + 1) + "." + (index + 1));
									...
									if (componentDT != null && componentDT.getChildren() != null) {
										if (j < componentDT.getChildren().size()) {
											...
											if (componentStr != null && !componentStr.equals("")) {
												Element componentElm = xmlDoc
														.createElement(segmentName + "." + (i + 1) + "." + (j + 1));
												Datatype subComponentDT = integrationProfile
														.findDatatypeById(component.getDatatypeId());
												if (subComponentDT.getChildren() == null
													...
													for (int k = 0; k < subComponentStrs.length; k++) {
														...
														if (subComponentStr != null && !subComponentStr.equals("")) {
															Element subComponentElm = xmlDoc.createElement(segmentName
																	+ "." + (i + 1) + "." + (j + 1) + "." + (k + 1));
	...
	private void generateSTDSegment(Element segmentElm, SegmentInstanceData instanceSegment, Document xmlDoc,
			IntegrationProfile integrationProfile) {
		String lineStr = instanceSegment.getLineStr();
		String segmentName = lineStr.substring(0, 3);
		Segment segment = instanceSegment.getSegmentDef();
...
//				}
							...
							if (fieldDatatype == null || fieldDatatype.getChildren() == null
								...
								for (int j = 0; j < componentStrs.length; j++) {
									if (componentDT.getChildren() != null && j < componentDT.getChildren().size()) {
										...
										if (componentStr != null && !componentStr.equals("")) {
											Element componentElm = xmlDoc
													.createElement(componentDataTypeName + "." + (j + 1));
											Datatype subComponentDT = integrationProfile
													.findDatatypeById(component.getDatatypeId());
											if (subComponentDT.getChildren() == null
													|| subComponentDT.getChildren().size() == 0) {
			...
			if (s.getDynamicMapping().getItems().size() > 0) {
				if (s.getDynamicMapping().getDynamicMappingDef().getPostion() != null
					...
					if (s.getDynamicMapping().getDynamicMappingDef().getReference() != null
						...
						dynamicInfo.setDynamicMappingTarget(s.getDynamicMapping().getDynamicMappingDef().getPostion());
						String referenceLocation = s.getDynamicMapping().getDynamicMappingDef().getReference();
						String secondReferenceLocation = s.getDynamicMapping().getDynamicMappingDef()
								.getSecondReference();
	...
	private List<FieldNode> popFields(Segment s, String segmentStr, SegmentParams params, DynamicInfo dynamicInfo,
			...
			for (int h = 0; h < fieldInstanceValues.size(); h++) {
				...
				if (params.getTestDataCategorizationMap() != null) {
					...
					if (fieldTestDataCategorizationObj != null) {
						fieldNode.setTestDataCategorization(fieldTestDataCategorizationObj.getTestDataCategorization());
						fieldNode.setTestDataCategorizationListData(fieldTestDataCategorizationObj.getListData());
					}
				}
				...
				if (fieldNode.getDt().getChildren() != null) {
					for (int j = 0; j < fieldNode.getDt().getChildren().size(); j++) {
						...
						if (params.getTestDataCategorizationMap() != null) {
							Categorization componentTestDataCategorizationObj = params.getTestDataCategorizationMap()
						...
						if (componentNode.getDt().getChildren() != null) {
							for (int k = 0; k < componentNode.getDt().getChildren().size(); k++) {
								...
								Datatype subComponentDt = profileData.getIntegrationProfile()
										.findDatatypeById(sc.getDatatypeId());
								...
								if (params.getTestDataCategorizationMap() != null) {
									Categorization subComponentTestDataCategorizationObj = params
											.getTestDataCategorizationMap()
	...
	private boolean comparePositionPath(String targetPosition, String predicatePosition, String targetType,
			String predicateLevel) {
	...
	private void analyzeSegmentRefOrGroup(SegmentRefOrGroup srog, int position, String positionPath,
		...
		if (srog instanceof SegmentRef) {
			...
			if (sr.getMax().equals("1") || isAnchor) {
				int index = 1;
				this.segmentsInfoList.add(this.generateSegmentInfo(index, s, sr, position, positionPath, iPositionPath,
						path, iPath, usagePath, isAnchor));
			} else {
				for (int i = 1; i < this.repeatedNum + 1; i++) {
					this.segmentsInfoList.add(this.generateSegmentInfo(i, s, sr, position, positionPath, iPositionPath,
							path, iPath, usagePath, isAnchor));
				}
			}
			...
			if (g.getMax().equals("1") || isAnchor) {
				...
				for (SegmentRefOrGroup child : g.getChildren()) {
					index = index + 1;
					if (index == 1) {
						this.analyzeSegmentRefOrGroup(child, index, positionPath, iPositionPath, path, iPath, usagePath,
								true, profileData);
					} else {
						this.analyzeSegmentRefOrGroup(child, index, positionPath, iPositionPath, path, iPath, usagePath,
								false, profileData);
	...
	private SegmentInfo generateSegmentInfo(int index, Segment s, SegmentRef sr, int position, String positionPath,
			String iPositionPath, String path, String iPath, String usagePath, boolean isAnchor) {
...
//        System.out.println(sInfo);        
	...
	private boolean isYourChild(SegmentInfo anchor, SegmentInfo current) {
		if (current.getiPositionPath().startsWith(this.removeLastPath(anchor.getiPositionPath())))
			return true;
		return false;
	}
